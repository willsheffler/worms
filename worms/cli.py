...
def build_worms_setup_from_cli_args(argv):
   ...
   if args.only_merge_bblocks == [-1]: args.only_merge_bblocks = []
   if args.only_bblocks == [-1]: args.only_bblocks = []
   if args.only_ivertex == [-1]: args.only_ivertex = []
   if args.bblock_ranges == [-1]: args.bblock_ranges = []
   elif args.shuffle_bblocks:
      print("you probably shouldnt use --shuffle_bblocks with --bblock_ranges ")
      sys.exit(0)
   if args.merge_segment == -1: args.merge_segment = None
   args.tolerance = min(args.tolerance, 9e8)

   if args.dbfiles == [""]:
      assert 0, "no --dbfiles specified"

   if len(args.nbblocks) == 1: args.nbblocks *= 100
   if args.output_only_connected != 'auto':
      if args.output_only_connected in ('', 0, '0', 'false', 'False'):
         args.output_only_connected = False
      else:
         args.output_only_connected = True

   kw = vars(args)
   if args.disable_cache: kw["db"] = NoCacheBBlockDB(**kw), NoCacheSpliceDB(**kw)
