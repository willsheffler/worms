...
def test_grow_cycle_thread_pool(c1pose):
    ...
    worms = grow(
        segments, Cyclic("C2", lever=20), executor=ThreadPoolExecutor, max_workers=2
    )
    assert 0.1411 < np.min(worms.scores) < 0.1412
...
def test_grow_cycle_process_pool(c1pose):
    ...
    worms = grow(
        segments, Cyclic("C2", lever=20), executor=ProcessPoolExecutor, max_workers=2
    )
    assert 0.1411 < np.min(worms.scores) < 0.1412
    assert np.sum(worms.scores < 0.1412) == 4
...
def test_multichain_match_reveres_pol(c1pose, c2pose):
    helix = Spliceable(c1pose, sites=[((1, 2, 3), "N"), ((9, 10, 11, 13), "C")])
    dimer = Spliceable(
        c2pose, sites=[("1,:1", "N"), ("1,-1:", "C"), ("2,:2", "N"), ("2,-1:", "C")]
    )
    segments = [
        Segment([helix], exit="C"),
        Segment([helix], entry="N", exit="C"),
...
def test_splicepoints(c1pose, c2pose, c3pose):
    ...
    dimer = Spliceable(
        c2pose, sites=[("1,:2", "N"), ("1,-1:", "C"), ("2,:2", "N"), ("2,-1:", "C")]
    )
    trimer = Spliceable(
        c3pose,
        sites=[
...
def test_invalid_splices_site_overlap_2(c1pose, c2pose):
    ...
    dimer = Spliceable(
        c2pose, sites=[("1,:1", "N"), ("2,:1", "N"), ("1,-1:", "C"), ("2,-1:", "C")]
    )
    segments = [
        Segment([helix], "_C"),
        Segment([dimer], "NN"),
    ...
    for i in range(len(w)):
        assert (
            w.segments[1].entrysiteid[w.indices[i, 1]]
            != w.segments[1].exitsiteid[w.indices[i, 1]]
        )
        assert (
            w.segments[3].entrysiteid[w.indices[i, 3]]
