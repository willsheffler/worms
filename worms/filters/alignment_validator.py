...
class AlignmentValidator:
    ...
    def test_pair_alignment(
        ...
        if (
            pose_info_n._pose.size() < superimpose_length
        ...
        residue_distances2 = [
            self.residue_ncac_avg_distance2(
                sse_n.residue(sse_index_n + x + 1), sse_c.residue(sse_index_c + x + 1)
            )
        ...
        for index in range(0, len(residue_distances2)):
            ...
            if superimpose_length <= index:
                ...
                sum_distance2 = (
                    sum_distance2 - residue_distances2[index - superimpose_length]
                )
                PRINTDBG("subtract %f" % residue_distances2[index - superimpose_length])
                assert subtract_index >= 0
            PRINTDBG("compute")
            if (
                superimpose_length <= index + 1
                and sum_distance2 < lowest_observed_distance2
            ):
