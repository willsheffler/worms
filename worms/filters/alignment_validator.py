...
class PoseInfo:
   ...
   def get_subpose(self, position0):
      if self._subposes[position0] is None:
         ...
         pose.append_subpose_to_pose(subpose, self._pose, start0 + 1, end0 + 1, False)
      ...
      if self._length_rhs is None:
         ...
         for position in range(len(self._dssp) - 2, 0, -1):
            if self._dssp[position] == self._dssp[position + 1]:
               self._length_rhs[position] = self._length_rhs[position + 1] + 1
   ...
   def get_run_length_lhs(self, position0):
      if self._length_lhs is None:
         ...
         for position in range(1, len(self._dssp)):
            if self._dssp[position] == self._dssp[position - 1]:
               self._length_lhs[position] = self._length_lhs[position - 1] + 1
...
class AlignmentValidator:
   ...
   def get_test_pair_alignment(
      ...
      if (pose_info_n._pose.size() < superimpose_length
          or pose_info_c._pose.size() < superimpose_length):
         print("failed size filter")
         return None, min(pose_info_n._pose.size(), pose_info_c._pose.size())
      ...
      if sse_shared_left + sse_shared_right + 1 < superimpose_length:
         print(
            "failed superimpose_length position index_n0,index_c0=%d,%d" % (index_n0, index_c0))
      ...
      self.align_pose_at_position1_sheffler(sse_n, sse_c, sse_index_n + 1, sse_index_c + 1)
      residue_distances2 = [
         self.residue_ncac_avg_distance2(sse_n.residue(sse_index_n + x + 1),
      ...
      for index in range(0, len(residue_distances2)):
         ...
         if superimpose_length <= index:
            ...
            sum_distance2 = (sum_distance2 - residue_distances2[index - superimpose_length])
            PRINTDBG("subtract %f" % residue_distances2[index - superimpose_length])
         ...
         if lowest_observed_distance2 <= max_sum_distance2:
            return "PASS ", np.sqrt(lowest_observed_distance2 / superimpose_length)
      # else:
   ...
   def residue_ncac_avg_distance2(self, res1, res2):
      ...
      for position in atom_positions:
         v1 = res1.xyz(position)  # what's the utility function for vector3.distance(v1, v2)?
         v2 = res2.xyz(position)
         err2 = err2 + (v1.x - v2.x)**2 + (v1.y - v2.y)**2 + (v1.z - v2.z)**2
