...
class ContactAnalyzer:
    ...
    def write_resfile(
        ...
        resfile_name = (
            output_pose.pdb_info().name() + ".resfile"
            if resfile_name is None
            else resfile_name.replace(".resfile", "") + ".resfile"
        )
    ...
    def get_lost_contacts(self, input_pose_map):
        ...
        lost_contacts = [
            i
            for i in range(1, input_pose_map.pose.size() + 1)
    ...
    def get_new_contacts(self, post_fusion_pose, subpose_final_ranges):
        ...
        result = [
            index
            for index in range(1, len(neighborhood_indices) + 1)
