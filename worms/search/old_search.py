...
def _get_chunk_end_seg(sizes, max_workers, memsize):
    ...
    while end > 1 and (
        util.bigprod(sizes[end:]) < max_workers
    ...
    *,
    ...
    if criteria.origin_seg is None:
        ...
        ntot, chunksize, nchunks = (
            util.bigprod(x) for x in (sizes, sizes[:end], sizes[end:])
        )
        ...
        if verbosity >= 0:
            print(
                "tot: {:,} chunksize: {:,} nchunks: {:,} nworker: {} njob: {}".format(
            ...
            print(
                "max_samples: {:,} max_results: {:,}".format(max_samples, max_results)
            )
        ...
        ntot, chunksize, nchunks = (
            util.bigprod(x)
            for x in (headsizes, headsizes[:headend], headsizes[headend:])
        )
        if max_samples is not None:
            max_samples = np.clip(chunksize * max_workers, max_samples, ntot)
        every_other = max(1, int(ntot / max_samples)) if max_samples else 1
        ...
        if xindex_cache_file and os.path.exists(xindex_cache_file):
            ...
            accum1 = MakeXIndexAccumulator(
                headsizes, from_seg=0, to_seg=-1, cart_resl=cart_resl, ori_resl=ori_resl
            )
            headcriteria = Cyclic(
                criteria[0].nfold,
                from_seg=0,
        ...
        ntot, chunksize, nchunks = (
            util.bigprod(x)
            for x in (tailsizes, tailsizes[:tailend], tailsizes[tailend:])
        )
        if max_samples is not None:
            max_samples = np.clip(chunksize * max_workers, max_samples, ntot)
        ...
        print(
            "STEP TWO: using xindex, nbins: {:,} nentries: {:,}".format(
...
def _grow_chunk(samp, segpos, conpos, context):
    ...
    ilow1 = (
        ilow0
        if (ML is None or ML >= ndimchunk)
        else ilow0[:ML] + (idxmap[ilow0[ML]],) + ilow0[ML + 1 :]
    )
...
def _check_topology(segments, criteria, expert=False):
    ...
    for a, b in zip(segments[:-1], segments[1:]):
        if not (a.exitpol and b.entrypol and a.exitpol != b.entrypol):
            raise ValueError(
                "incompatible exit->entry polarity: "
                + str(a.exitpol)
                + "->"
                + str(b.entrypol)
                + " on segment pair: "
