...
from worms.search import grow_linear, lossfunc_random_1_in_1000
...
def perf_grow_3(bbdb, maxbb=10, shuf=0):
    ...
    v = Vertex(vbbs, 'CC', min_seg_len=15, parallel=1)
    w = Vertex(wbbs, 'N_', min_seg_len=15, parallel=1)
    verts = (u, v, w)
    ...
    f = Edge(v, vbbs, w, wbbs, parallel=1)
    edges = (e, f)
    tedge = time() - tedge
    ...
    for par in (0, 1):
        ...
        worms = grow_linear(
            verts,
            edges,
            loss_function=lossfunc_random_1_in_1000,
            parallel=par)
        tgrow = time() - tgrow
        Nres = len(worms.losses)
        Ntot = u.len * v.len * w.len
        ttot = 0  # time() - ttot
